...
public class DBGenerator implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Bill bill = new Bill();
        bill.setCode("HD0001");
        bill.setAddress("Tu Hoàng, Xuân Phương, Namm Từ Liêm, Hà Nội");
        bill.setPhoneNumber("0981617258");
        bill.setCustomer(account4);
        bill.setEmployee(account1);
        bill.setCustomerName("Vũ Quang Huy");
        bill.setConfirmationDate(convertLong("01/06/2003"));
        bill.setDeliveryDate(convertLong("01/06/2003"));
        bill.setReceivedDate(convertLong("05/06/2003"));
        bill.setCompletionDate(convertLong("05/06/2003"));
        bill.setMoneyShip(new BigDecimal(30000));
        bill.setTotalMoney(new BigDecimal(3430000));
        bill.setStatus(OrderStatus.DA_HOAN_THANH);
        bill.setCustomerInformation(customerInformation);
        bill.setId(orderRepository.save(bill).getId());

        Bill bill1 = new Bill();
        bill1.setCode("HD0002");
        bill1.setAddress("Hòa Lạc, Hà Nội");
        bill1.setPhoneNumber("0328843156");
        bill1.setCustomer(account3);
        bill1.setEmployee(account1);
        bill1.setCustomerName("Đặng Ngọc Anh");
        bill1.setConfirmationDate(convertLong("01/06/2003"));
        bill1.setDeliveryDate(convertLong("01/06/2003"));
        bill1.setReceivedDate(convertLong("05/06/2003"));
        bill1.setCompletionDate(convertLong("05/06/2003"));
        bill1.setMoneyShip(new BigDecimal(50000));
        bill1.setTotalMoney(new BigDecimal(3050000));
        bill1.setStatus(OrderStatus.DA_HOAN_THANH);
        bill1.setCustomerInformation(customerInformation1);
        bill1.setId(orderRepository.save(bill1).getId());

        orderRepository.save(bill);
        orderRepository.save(bill1);

        BillDetail billDetail = new BillDetail();
        billDetail.setQuantity(1);
        billDetail.setPrice(new BigDecimal(3400000));
        billDetail.setProductDetail(productDetail);
        billDetail.setBill(bill);
        billDetail.setStatus(OrderDetailStatus.DA_BAN);

        BillDetail billDetail1 = new BillDetail();
        billDetail1.setQuantity(1);
        billDetail1.setPrice(new BigDecimal(3000000));
        billDetail1.setProductDetail(productDetail3);
        billDetail1.setBill(bill1);
        billDetail1.setStatus(OrderDetailStatus.DA_BAN);

        orderDetailRepository.save(billDetail);
        orderDetailRepository.save(billDetail1);

        BillHistory billHistory = new BillHistory();
        billHistory.setActionDescription("Mua hàng");
        billHistory.setBill(bill);
        billHistory.setStatus(OrderStatus.DA_HOAN_THANH);

        BillHistory billHistory1 = new BillHistory();
        billHistory1.setActionDescription("Mua hàng");
        billHistory1.setBill(bill1);
        billHistory1.setStatus(OrderStatus.DA_HOAN_THANH);

        orderHistoryRepository.save(billHistory);
        orderHistoryRepository.save(billHistory1);
